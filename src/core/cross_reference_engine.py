...
def detect_outlier_salarial(conn: duckdb.DuckDBPyConnection) -> list[Alert]:
    ...
    try:
        df = conn.execute("""
            ...
            SELECT
                s.split_part(servidor, ' - ', 2),
                s.cargo,
                s.secretaria,
                CAST(s.salario_liquido AS DOUBLE) AS salario,
                st.media,
                st.desvio,
                (CAST(s.salario_liquido AS DOUBLE) - st.media) / NULLIF(st.desvio, 0) AS zscore
            FROM servidores s
            JOIN stats st ON s.cargo = st.cargo
            WHERE (CAST(s.salario_liquido AS DOUBLE) - st.media) / NULLIF(st.desvio, 0) > 3
    ...
    for _, row in df.iterrows():
        ...
        alerts.append(Alert(
            ...
            entity_name=row.get("split_part(servidor, ' - ', 2)", ""),
            description=(
    ...
    try:
        df = conn.execute("""
            SELECT
                ...
                STRING_AGG(split_part(servidor, ' - ', 2), ' | ') AS servidores,
                secretaria
            FROM diarias
...
def detect_doacao_to_contrato(conn: duckdb.DuckDBPyConnection) -> list[Alert]:
    ...
    try:
        df = conn.execute("""
            SELECT
                d.nm_doador_originario,
                d.nr_cpf_cnpj_doador_originario,
                d.nm_doador_originario,
                d.vr_receita AS valor_doacao,
                d.dt_receita AS data_doacao,
                o.empresa_nome,
                SUM(CAST(o.valor_total AS DOUBLE)) AS contratos_pos_eleicao
            FROM tse_doacoes d
            JOIN obras o ON (
                d.nr_cpf_cnpj_doador_originario = o.empresa_id::VARCHAR
                OR o.empresa_nome ILIKE '%' || SPLIT_PART(d.nm_doador_originario, ' ', 1) || '%'
            )
            WHERE o.capturado_em > '2024-10-06'  -- dia da eleição 2024
              AND d.dt_receita < '2024-10-07'
            GROUP BY
                d.nm_doador_originario, d.nr_cpf_cnpj_doador_originario,
                d.nm_doador_originario, d.vr_receita, d.dt_receita,
                o.empresa_nome
    ...
    for _, row in df.iterrows():
        alerts.append(Alert(
            ...
            entity_name=row["empresa_nome"],
            description=(
                f"{row['nm_doador_originario']} doou R${float(row['valor_doacao']):,.2f} "
                f"para {row['nm_doador_originario']} em {row['data_doacao']}. "
...
def detect_fim_de_semana(conn: duckdb.DuckDBPyConnection) -> list[Alert]:
    ...
    for _, row in df.iterrows():
        ...
        alerts.append(Alert(
            ...
            severity="MÉDIO",
            entity_type="servidor",
            entity_name=row["split_part(servidor, ' - ', 2)"],
            description=(
...
def detect_nepotismo_sobrenome(conn: duckdb.DuckDBPyConnection) -> list[Alert]:
    ...
    for _, row in df.iterrows():
        alerts.append(Alert(
            ...
            entity_name=row["split_part(servidor, ' - ', 2)"],
            description=(
                f"Servidor {row['split_part(servidor, ' - ', 2)']} ({row['secretaria']}) compartilha "
