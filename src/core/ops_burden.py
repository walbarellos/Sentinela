...
def _build_sesacre_burden(case: dict[str, Any], artifacts_df: pd.DataFrame, inbox_df: pd.DataFrame) -> list[dict[str, Any]]:
    ...
    return [
        _make_burden_row(
            ...
            item_label="Fornecedor com sancao ativa e contratacao concomitante",
            status="COMPROVADO_DOCUMENTAL",
            evidence_grade="DOCUMENTAL_CORROBORADO",
            anchors=["CF88_ART37_CAPUT", "L12846_2013", "D11129_2022"],
            source_refs=_artifact_sources(artifacts_df, "dossie_sesacre_top10", "noticia_fato_top10"),
            rationale="O caso ja esta materializado no cruzamento sancionatorio colapsado e dossie estadual.",
            next_action="Preservar cadeia de evidencia do cruzamento e do recorte financeiro.",
        ),
        _make_burden_row(
            case_id=case["case_id"],
            family=case["family"],
...
def _build_saude_societario_burden(case: dict[str, Any], artifacts_df: pd.DataFrame, inbox_df: pd.DataFrame) -> list[dict[str, Any]]:
    return [
        ...
        _make_burden_row(
            ...
            family=case["family"],
            item_key="nepotismo",
            item_label="Nepotismo ou parentesco vedado",
            status="SEM_BASE_ATUAL",
            evidence_grade="SEM_LASTRO",
            anchors=["CF88_ART37_CAPUT"],
            source_refs=[],
            rationale="O caso nao tem hoje documento publico suficiente para afirmar parentesco vedado ou designacao cruzada.",
            next_action="Nao promover essa tese sem documento nominal e vinculacao funcional verificavel.",
        ),
    ]
