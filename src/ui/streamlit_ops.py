...
def render_ops_page() -> None:
    ...
    with toolbar_far:
        if st.button("🔎 Reindexar Texto", use_container_width=True):
            try:
                ...
                st.cache_data.clear()
                st.success(f"Índice textual atualizado: {stats['indexed_docs']} docs.")
                st.rerun()
            except Exception as exc:
                st.error(f"Falha ao reindexar texto: {exc}")
    ...
    overview_tab, cases_tab, search_tab, inbox_tab, runtime_tab = st.tabs(["Visão Geral", "Bancada de Casos", "Busca", "Inbox", "Runtime"])
    with overview_tab:
